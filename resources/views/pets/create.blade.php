            ...
            <form method="POST" action="{{ url('pets') }}" class="flex flex-col md:flex-row gap-4 mt-4" enctype="multipart/form-data">
                ...
                <div class="w-full md:w-[320px]">
                    <div class="avatar flex flex-col gap-1 items-center justify-center cursor-pointer hover:scale-105 transition ease-in">
                        <div id="upload" class="mask mask-squircle bg-white w-48">
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="name" placeholder="Firulais" value="{{ old('name') }}">
                    @error('name')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Kind --}}
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="weight" placeholder="4.5" value="{{ old('weight') }}">
                    @error('weight')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Age --}}
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="breed" placeholder="Pug" value="{{ old('breed') }}">
                    @error('breed')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Location --}}
                    <label class="label text-white">Location:</label>
                    <input type="text" class="input bg-[#0009] outline-0" name="location" placeholder="Tokio" value="{{ old('location') }}">
                    @error('location')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Description --}}
                    <label class="label text-white">Description:</label>
                    <textarea name="description" class="textarea bg-[#0009] outline-0" rows="4" placeholder="Lorem ipsum dolor">{{ old('description') }}</textarea>
                    @error('description')
